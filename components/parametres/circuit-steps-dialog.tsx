...
import type { Circuit, CircuitStep } from "@/hooks/circuit/type"
...
export function CircuitStepsDialog({
  ...
  const { data: steps, isLoading } = useCircuitSteps(circuit.id)
  ...
  function handleSave() {
    ...
    if (editingId) {
      ...
      createStep.mutate({ circuitId: circuit.id, ...body }, { onSuccess, onError })
    }
  }
